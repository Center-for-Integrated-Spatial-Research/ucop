...
var campusLargeUrl = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/0";
var campusSmallFeatureLayer;
var medCenterSmallFeatureLayer;
var labSmallFeatureLayer;
var campusSmallUrl = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/1";
var campusNames;
var reservesFeatureLayer;
var reservesUrl = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/2";
var communityProgramDynamicLayer;
var communityProgramDynamicLayerUrl = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOPCommunityPrograms/MapServer";
var communityProgramFeatureLayer577k;
var communityProgramUrl577k = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/3";
var communityProgramFeatureLayer288k;
var communityProgramUrl288k = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/4";
var communityProgramFeatureLayer144k;
var communityProgramUrl144k = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/5";
var communityProgramFeatureLayer72k;
var communityProgramUrl72k = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/6";

var assemblyFeatureLayer;
var assemblyUrl = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/7"
var assemblyDistricts;
//var senateFeatureLayer;
//var senateUrl = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/8"
//var senateDistricts;
var congressFeatureLayer;
var congressUrl = "http://arcgis.cisr.ucsc.edu/arcgis/rest/services/UCOP/UCOP_2015_01/MapServer/9"
...
var navigationTool;
var addressInfo = {"address":"", "assembly":"", "congress":"", "campus":""}
...
function init(){
	...
	//esri.config.defaults.io.corsEnabledServers.push("http://arcgis.cisr.ucsc.edu");
...
function queryByLayer(value) {
	...
	/*else if (layer == "senate") {
		senateFeatureLayer.selectFeatures(query, esri.layers.FeatureLayer.SELECTION_NEW, function(results) {
			queryCommunityPrograms( "SenateDistrict", results[0].attributes["District_Name"].slice(-2) )
			queryReserves( "SenateDistrict", results[0].attributes["District_Name"].slice(-2) )
			queryCampus( "SenateDistrict", results[0].attributes["District_Name"].slice(-2) )
			zoomToFeature(results[0].geometry);
			showDistrictResults(results[0].attributes);
...
function getSpatialQueryResults(name,geometry) {
	...
	deferredList.then(function(results) {
		...
		addressInfo.congress = results[1][1].features[0].attributes["District_Name"];
		var distances = [];
		dojo.forEach(results, function(result,i) {
			if (i>1) { distances.push(result[1]) }
		});
...
function populateDistrictTable(a) {
	...
	if (layer == "assembly"){	
		...
		var fields = ["Campus_Legislator", "Student_Count","Total_Alumni", "Total_Employee_Count", "Retiree_Count", "Licensee_Count",  "Salary_Gross", "Med_Center", "National_Lab", "Cal_Grant_Recipients", "Cal_Grant_Amount"]
		var person = "Assembly_Person";
		var personTitle = "Assembly Person";
	} 
	/*else if (layer == "senate") {
		dojo.forEach(senateFeatureLayer.fields, function(field) { layerFields[field.name] =  field.alias });
		var fields = ["Campus", "Student_Count", "Total_Employee_Count", "Alumni_Count_First_Half", "Alumni_Count_Second_Half", "Retiree_Count", "Licensee_Count", "Salary_Gross", "Pell_Grant_Amount"];
		var person = "Senate_Person";
	}*/ 
	else if (layer == "congress") {
		dojo.forEach(congressFeatureLayer.fields, function(field) { layerFields[field.name] =  field.alias });
		var fields = ["Campus", "Student_Count", "Total_Alumni",  "Total_Employee_Count", "Retiree_Count", "Licensee_Count", "Licensee_Count_Campus", "Total_Salary", "Med_Center", "National_Lab", "Pell_Grant_Recipient_Count", "Pell_Grant_Amount", "Federal_Loan_Recipients", "Federal_Loan_Amount"];
		var person = "Congress_Person";
	...
	dojo.forEach(fields, function(field,i) {
		...
		if (value != null) {
			...
			if (field == "Student_Count") {
				content += "<tr><td colspan=2></td><td></td></tr>";
				content +="<tr><td colspan=2 class=\"resultsTableHeaderText\">Undergraduates</td><td class=\"resultsTableAmounts\">" + value  + "</td></tr>"
			}
			...
			if ((field == "Cal_Grant_Amount") && (layer == "assembly"))  {
				content += "<tr><td colspan=2></td><td></td></tr>";
...
function populateCampusTable(a) {
	var fields = ["Students", "Undergraduates", "Employees", "Retirees", "Alumni", "UC_Payroll", "Outpatient_Visits", "Cal_Grant_Recipients", "Cal_Grant_Amount", "Pell_Grant_Recipients", "Pell_Grant_Amount", "Federal_Loan_Recipients", "Federal_Loan_Amount", "Funding_NIH", "Funding_USDA", "Funding_NSF", "Funding_DOE", "Funding_DOD", "Funding_NASA", "Funding_Other"];
	...
	var content = ""
	if (a["Students"] != null) {
		content += "<tr><td colspan=2 class=\"resultsTableHeaderText\">Students</td><td class=\"resultsTableAmounts\">" + dojo.number.format(a["Students"].toFixed(0))  + "</td></tr>"
	}
	...
	content +="<tr><td colspan=2 class=\"resultsTableHeaderText\">Employees</td><td class=\"resultsTableAmounts\">" + dojo.number.format(a["Employees"].toFixed(0))  + "</td></tr>"
	content +="<tr><td colspan=2></td><td></td></tr>"
	content +="<tr><td colspan=2 class=\"resultsTableHeaderText\">Retirees</td><td class=\"resultsTableAmounts\">" + dojo.number.format(a["Retirees"].toFixed(0))  + "</td></tr>"
	content +="<tr><td colspan=2></td><td></td></tr>"	
	content +="<tr><td colspan=2 class=\"resultsTableHeaderText\">Alumni</td><td class=\"resultsTableAmounts\">" + dojo.number.format(a["Alumni"].toFixed(0))  + "</td></tr>"
	content +="<tr><td colspan=2></td><td></td></tr>"
	content +="<tr><td colspan=2 class=\"resultsTableHeaderText\">UC Payroll</td><td class=\"resultsTableAmounts\">$" + dojo.number.format(a["UC_Payroll"].toFixed(0))  + "</td></tr>"	
...
function showMapPdf() {
	var layer = dijit.byId('layer').get('value');
	var value = dijit.byId('district').get('value');
	if ((layer == 'assembly') || (layer == 'congress')) {
		var prefix = (layer == 'assembly') ? "AD" : "CD";
